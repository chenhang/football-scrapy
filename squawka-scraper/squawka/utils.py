...
class SquawkaReport:
    ...
    def _parse_elements(self, elements):
        ...
        for e in elements:
            ...
            for c in e.getchildren():
                event = dict([('_'.join([c.tag, k]), v)
                              for (k, v) in c.attrib.items()] +
                             time_slice.items() + [(c.tag, c.text)])
                for cc in c.getchildren():
                    event = dict([('_'.join([cc.tag, k]), v)
    ...
    @property
    def kickoff(self):
        date = self.xml.xpath("/squawka/data_panel/game/kickoff/text()")
        if len(date) > 0:
            return parser.parse(date[0]).strftime('%Y-%m-%d %H:%M:%S %z')
        else:
            return self.path
...
def export_all_stats(xml_dir,
    ...
    for statistic in statistics:
        if statistic in sequential:
            df = pd.concat(
                (_load_xml(p, statistic) for p in xml_paths),
                axis=0,
                ignore_index=True)
        else:
...
def split_coordinates(s):
    ...
    split = pd.DataFrame(
        concatenated.str.split(',').tolist(), columns=['x', 'y'], dtype=float)
